...
export default function DetailTrendingBlog() {
  ...
  useEffect(() => {
    ...
    sanityClient
      .fetch(
        `*[_type == "detailTrendingBlogHeader"] {
          heading,
          mainImage{
            asset->{
              url
  ...
  return (
    <section className="mt-[4rem] mx-auto w-[95%] max-w-[1150px]">
      {header && (
        <>
        ...
        blogs.map((blog, index) => {
          return (
            <article className="flex flex-col gap-12 mb-16" key={index}>
              ...
              <img
                className="px-4 md:px-0 mx-auto rounded-2xl max-w-[27rem] object-cover aspect-video"
                src={blog.mainImage}
